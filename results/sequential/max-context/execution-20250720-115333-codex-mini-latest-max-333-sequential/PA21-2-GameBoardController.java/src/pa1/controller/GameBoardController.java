...
public class GameBoardController {
	...
	public MoveResult makeMove(final Direction direction) {
		final MoveResult result = tryMove(gameBoard.getPlayer().getPosition(), direction);
		if (result instanceof MoveResult.Valid.Alive alive) {
			final Position oldPos = alive.origPosition;
			gameBoard.board[oldPos.row()][oldPos.col()] = new StopCell();
			...
			for (Position lifePos : alive.collectedExtraLives) {
				gameBoard.board[lifePos.row()][lifePos.col()] = new StopCell();
			}
			final Position newPos = alive.newPosition;
			final EntityCell playerCell = gameBoard.getEntityCell(oldPos);
			playerCell.getEntity().setPosition(newPos);
			gameBoard.board[newPos.row()][newPos.col()] = playerCell;
			return alive;
		}
		return result;
	}
	...
	public void undoMove(final MoveResult prevMove) {
		...
		final Position landing = alive.newPosition;
		gameBoard.getEntityCell(landing).setEntity(null);
		gameBoard.getCell(landing.row(), landing.col());
		gameBoard.getEntityCell(landing).setEntity(null);
		gameBoard.getEntityCell(landing).setEntity(null);
		final Position origin = alive.origPosition;
		gameBoard.getEntityCell(origin).setEntity(gameBoard.getPlayer());
		for (Position gemPos : alive.collectedGems) {
			gameBoard.getEntityCell(gemPos).setEntity(new Gem(gemPos));
		}
		for (Position lifePos : alive.collectedExtraLives) {
			gameBoard.getEntityCell(lifePos).setEntity(new ExtraLife(lifePos));
		}
	}
}
