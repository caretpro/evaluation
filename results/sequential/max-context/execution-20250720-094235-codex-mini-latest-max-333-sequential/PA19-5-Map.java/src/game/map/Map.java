...
public class Map {
    ...
    public boolean hasLost() {
        // If we've never called fillTiles, prevFilledDistance is still null → can't have lost yet.
        if (prevFilledDistance == null) {
            return false;
        }
        // If in the last fillTiles call no pipes were filled, we have lost.
    ...
    public Map(int rows, int cols) {
        ...
        this.sourceCell = new TerminationCell(startInfo.coord, startInfo.dir, TerminationCell.Type.SOURCE);
        this.cells[startInfo.coord.row][startInfo.coord.col] = sourceCell;
        var endInfo = generateEndCellInfo();
        this.sinkCell = new TerminationCell(endInfo.coord, endInfo.dir, TerminationCell.Type.SINK);
                ...
                if (cell instanceof TerminationCell tc) {
                    if (tc.type == TerminationCell.Type.SOURCE) {
                        ...
                        if (adj.row < 0 || adj.row >= rows || adj.col < 0 || adj.col >= cols
                                || this.cells[adj.row][adj.col] instanceof Wall) {
     ...
     * <p>
     * Hint: There are two ways to approach this. You can either iteratively fill the tiles by
     * distance (i.e. filling distance=0, distance=1, etc), or you can save the tiles you have already
     * filled, and fill all adjacent cells of the already-filled tiles. Whichever method you choose
     * is up to you, as long as the result is the same.
     * </p>
     *
     ...
     * <p>
     * The game is won when the player must place pipes on the map such that a path is formed
     * from the source tile to the sink tile. One of the approaches to check this is to use
     * Breadth First Search to search for the sink tile along the pipes. You may also use
     * other algorithms or create your own, provided it achieves the same goal. The game is
     * lost when no additional pipes are filled in each round after the Nth round. The value
     * of N` can be configured in the loaded map. In the example maps, N` is set to 10.
     * </p>
     *
