...
public class Game {
    ...
    /**
     * Constructs a game with an empty map of the given size.
    ...
    /**
     * Undos a step from the game. <p> Note: Undoing a step counts will increment the number of steps by one. </p>
     * @return   {@code  false}  if there are no steps to undo, otherwise  {@code  true} .
     */
    public boolean undoStep() {
        FillableCell last = cellStack.pop();
        if (last == null || last.getPipe().isEmpty()) {
            return false;
        }
        map.undo(last.coord);
