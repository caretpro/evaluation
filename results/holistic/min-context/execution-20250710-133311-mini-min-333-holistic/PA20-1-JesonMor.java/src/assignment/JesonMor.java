...
public class JesonMor extends Game {
	...
	public Player start() {
		...
		while (true) {
			...
			// If no moves available, skip turn or end game? Here we skip turn.
			if (availableMoves.length == 0) {
				// No moves for current player, next player
	...
	public Player getWinner(Player lastPlayer, Piece lastPiece, Move lastMove) {
		// A player wins if the last moved piece is a Knight and it reaches the opponent's back row
		// Assuming board height is board[0].length or board.length depending on orientation
		// Let's assume board.length is width (x), board[0].length is height (y)
		// We check if lastPiece is Knight and if lastMove destination is in opponent's back row

		if (lastPiece instanceof Knight) {
			int destY = lastMove.getDestination().y();
			Player[] players = configuration.getPlayers();

			// Assuming two players only
			if (players.length == 2) {
				Player opponent = players[0] == lastPlayer ? players[1] : players[0];
				// Check if lastMove destination is opponent's back row
				// If lastPlayer is players[0], opponent's back row is last row (max y)
				// If lastPlayer is players[1], opponent's back row is first row (y=0)

				if (lastPlayer == players[0]) {
					// opponent back row is last row
					if (destY == board[0].length - 1) {
						return lastPlayer;
					}
				} else {
					// opponent back row is first row
					if (destY == 0) {
						return lastPlayer;
					}
				}
	...
	public void updateScore(Player player, Piece piece, Move move) {
		...
		int manhattanDistance = Math.abs(dest.x() - src.x()) + Math.abs(dest.y() - src.y());
