...
public class ConsolePlayer extends Player {
    ...
    public ConsolePlayer(String name, assignment.protocol.Color color) {
        super(name, color);
    }

    public ConsolePlayer(String name) {
        this(name, assignment.protocol.Color.GREEN);
    }
    ...
    public @NotNull Move nextMove(Game game, Move[] availableMoves) {
        ...
        while (true) {
            ...
            System.out.printf("%s, enter your move (e.g. a1->b2): ", getName());
            String line = scanner.nextLine().strip();

            Move chosen = parseMove(line);
            if (chosen == null) {
                System.out.println("Invalid input format; please use 'a1->b2' style.");
                continue;
            }

            boolean inList = Arrays.stream(availableMoves).anyMatch(m -> m.equals(chosen));
            if (!inList) {
            ...
            String error = validateMove(game, chosen);
            if (error != null) {
