...
public class Map {
    ...
    public Map(int rows, int cols, Cell[][] cells) {
        ...
        // Copy and locate termination cells
        TerminationCell foundSource = null, foundSink = null;
        for (int r = 0; r < rows; r++) {
            if (cells[r].length != cols) {
                throw new IllegalArgumentException("All rows must have length " + cols);
            }
            for (int c = 0; c < cols; c++) {
                Cell cell = cells[r][c];
                this.cells[r][c] = cell;
                if (cell instanceof TerminationCell tc) {
                    if (tc.isSource()) {
                        if (foundSource != null) {
                            throw new IllegalArgumentException("Multiple sources");
                        }
                        foundSource = tc;
                    } else {
                        if (foundSink != null) {
                            throw new IllegalArgumentException("Multiple sinks");
                        }
                        foundSink = tc;
                    }
                }
        ...
        if (foundSource == null || foundSink == null) {
            throw new IllegalArgumentException("Map must have exactly one source and one sink");
        }
    ...
    public void undo(final Coordinate coord) {
        ...
        if (!(cell instanceof FillableCell fillable)) {
            throw new IllegalArgumentException("Can't undo on non-fillable cell");
        }
        fillable.removePipe();
    }
    ...
    public void fillTiles(int distance) {
        ...
        // BFS from sourceCell up to depth=distance
        ...
        while (!queue.isEmpty()) {
            ...
            if (!(c instanceof FillableCell fc) || !fc.isFilled()) {
                continue;
            }
            for (Direction dir : Direction.values()) {
                ...
                if (nc instanceof FillableCell nfc && !nfc.isFilled()) {
                    if (fc.canFlowTo(dir, nfc)) {
                        nfc.setFilled();
    ...
    public boolean checkPath() {
        ...
        while (!queue.isEmpty()) {
            ...
            for (Direction dir : Direction.values()) {
                ...
                Cell nc = cells[next.row][next.col];
                if (nc instanceof FillableCell nfc && nfc.isFilled() && !visited.contains(next)) {
                    if (fc.canFlowTo(dir, nfc)) {
                        visited.add(next);
                        queue.add(next);
                    }
                }
                if (nc instanceof TerminationCell tc && !tc.isSource()) {
                    // can we connect to sink?
                    if (fc.canFlowTo(dir, tc)) {
                        return true;
                    }
                }
            }
    ...
    public boolean hasLost() {
        // lost if no new tiles filled this round
