 ...
 * </p>
 * <ul>
 *     <li>There must be at most one entity owned by each cell; In other words,
 *     there should never be two entities'
 *     {@link Entity#getOwner()} pointing to the same {@link EntityCell}.</li>
 *     <li>There must be at most one cell owning each entity; In other words,
 *     there should never be two cells'
...
public class EntityCell extends Cell {
    ...
    public Entity setEntity(final Entity newEntity) {
        ...
        // 1) Detach our old entity, if any:
        Entity old = this.entity;
        if (old != null) {
            old.setOwner(null);
        }

        // 2) If newEntity already lives somewhere else, detach it from its old owner:
        EntityCell prevOwner = newEntity.getOwner();
        if (prevOwner != null) {
            prevOwner.entity = null;
        }

        // 3) Claim the new entity:
