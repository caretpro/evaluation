...
public class GameMap {
     ...
     * <p>
     * Notes:
     * <li>There can be at most 26 players.
    ...
    public static GameMap parse(String mapText) {
        ...
        if (lines.length < 2) {
            throw new IllegalArgumentException("Map text must have at least two lines");
        }
        ...
        Set<Character> seenPlayers = new HashSet<>();
        AtomicInteger row = new AtomicInteger(0);

        Map<Character, List<Position>> boxPositions = new HashMap<>();

        for (int y = 1; y < lines.length; y++) {
            String line = lines[y];
            for (int x = 0; x < line.length(); x++) {
                char c = line.charAt(x);
                Position pos = new Position(x, row.get());
                switch (c) {
                    case '#':
                    ...
                    default:
                        if (Character.isUpperCase(c)) {
                            ...
                            boxPositions.computeIfAbsent(c, k -> new ArrayList<>()).add(pos);
                        } else {
        ...
        int totalBoxes = boxPositions.values().stream().mapToInt(List::size).sum();
        if (totalBoxes != destinations.size()) {
            throw new IllegalArgumentException("Number of boxes must equal number of destinations");
        }

        // Ensure each player has at least one box and each box type has its player
        for (char pc : seenPlayers) {
            char lc = Character.toLowerCase(pc);
            if (!boxPositions.containsKey(lc)) {
                throw new IllegalArgumentException("Player " + pc + " has no corresponding boxes");
            }
        }
        for (char bc : boxPositions.keySet()) {
            char uc = Character.toUpperCase(bc);
            if (!seenPlayers.contains(uc)) {
                throw new IllegalArgumentException("Box " + bc + " has no matching player");
            }
        }
    ...
    public Entity getEntity(Position position) {
        return Objects.requireNonNull(map.get(position),
                "No entity at position " + position);
    }
    ...
    public void putEntity(Position position, Entity entity) {
        // map was unmodifiable in the privateCtor, but for the mutable ctor we used a plain HashMap
