...
public class Game {
	...
	// To keep track of last placed pipe for undo
	private Pipe lastUsedPipe = null;
	...
	public boolean placePipe(int row, char col) {
		...
		boolean placed = map.setPipe(coordinate, currentPipe);
		if (placed) {
			pipeQueue.poll();
			delayBar.tick();
	...
	public boolean undoStep() {
		...
		map.removePipe(lastCell);
		if (lastUsedPipe != null) {
			pipeQueue.undo(lastUsedPipe);
			lastUsedPipe = null;
		}
		numOfSteps++;
		return true;
