...
public abstract class AbstractSokobanGame implements SokobanGame {
    ...
    protected boolean shouldStop() {
        // stop when exit was requested or when all boxes are on destinations
    ...
    protected ActionResult processAction(Action action) {
        if (action instanceof InvalidInput invalid) {
            // direct pass‑through for invalid inputs
            return new ActionResult.Failed(invalid, invalid.getMessage());
        }
        if (action instanceof Exit) {
            // mark exit and report exit
            isExitSpecified = true;
            return new ActionResult.Exit();
        }
