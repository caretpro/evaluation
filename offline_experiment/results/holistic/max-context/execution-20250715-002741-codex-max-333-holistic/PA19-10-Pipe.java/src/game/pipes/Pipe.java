...
/**
 * A pipe segment on the map.  Pipes have a fixed {@link Shape}, can be {@linkplain #setFilled() filled}, and
 * expose their connection directions and single‐character representation for rendering.
 */
public class Pipe implements MapElement {
    ...
    public Direction[] getConnections() {
        char rep = shape.getCharByState(filled);
        Direction[] conns = PipePatterns.getConnections(rep);
        if (conns == null) {
            throw new IllegalStateException(
                "Unknown pipe pattern for shape " + shape + " (filled=" + filled + ")"
            );
        }
        return conns;
    }
    ...
    public static Pipe fromString(String rep) {
        ...
        switch (rep.toUpperCase(Locale.ROOT)) {
            case "HZ": return new Pipe(Shape.HORIZONTAL);
            case "VT": return new Pipe(Shape.VERTICAL);
            case "TL": return new Pipe(Shape.TOP_LEFT);
            case "TR": return new Pipe(Shape.TOP_RIGHT);
            case "BL": return new Pipe(Shape.BOTTOM_LEFT);
            case "BR": return new Pipe(Shape.BOTTOM_RIGHT);
            case "CR": return new Pipe(Shape.CROSS);
            default:
                throw new IllegalArgumentException(
                    "Unknown pipe code '" + rep + "'. Valid codes: "
                  + Arrays.toString(Arrays.stream(Shape.values())
                                           .map(Enum::name)
                                           .toArray(String[]::new))
                );
        }
    ...
    /**
     * All supported pipe shapes, each with its filled‐ and unfilled‐glyph.
     */
    public enum Shape {
