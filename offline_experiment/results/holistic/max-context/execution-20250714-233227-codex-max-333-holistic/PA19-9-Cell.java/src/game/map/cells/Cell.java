...
public abstract class Cell implements MapElement {
    ...
    public static Cell fromChar(char c, Coordinate coord, TerminationCell.Type terminationType) {
        return switch (c) {
            ...
            case '^' -> (terminationType == TerminationCell.Type.NORTH)
                    ? new TerminationCell(coord, TerminationCell.Type.NORTH)
                    : null;
            case 'v' -> (terminationType == TerminationCell.Type.SOUTH)
                    ? new TerminationCell(coord, TerminationCell.Type.SOUTH)
                    : null;
            case '<' -> (terminationType == TerminationCell.Type.WEST)
                    ? new TerminationCell(coord, TerminationCell.Type.WEST)
                    : null;
            case '>' -> (terminationType == TerminationCell.Type.EAST)
                    ? new TerminationCell(coord, TerminationCell.Type.EAST)
                    : null;
            default -> null;
        };
    }
