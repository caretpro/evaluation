...
public class GameMap {
    ...
    public static GameMap parse(String mapText) {
        ...
        if (lines.length < 2) {
            throw new IllegalArgumentException("Map text must have at least two lines (undo limit + map).");
        }
        ...
        // ensure each box type has a matching player, and vice versa
        for (Integer pid : boxes.keySet()) {
            if (!players.containsKey(pid)) {
                throw new IllegalArgumentException("Box for player " + (char)('A' + pid) + " has no matching player.");
            }
        }
        for (Integer pid : players.keySet()) {
            if (!boxes.containsKey(pid)) {
                throw new IllegalArgumentException("Player " + (char)('A' + pid) + " has no matching boxes.");
            }
        }
