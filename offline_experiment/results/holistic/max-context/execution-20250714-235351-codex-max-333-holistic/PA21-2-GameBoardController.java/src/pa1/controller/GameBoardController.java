...
public class GameBoardController {
    ...
    public MoveResult makeMove(final Direction direction) {
        ...
        // Determine the outcome without mutating the board
        MoveResult result = tryMove(gameBoard.getPlayer().getOwner().getPosition(), direction);

        // If the move is invalid or kills the player, do not change the board
        if (result instanceof MoveResult.Invalid || result instanceof MoveResult.Valid.Dead) {
            return result;
        }

        // Otherwise it's a valid & alive move: commit the changes
        ...
        // 1) Remove player from orig
        gameBoard.getEntityCell(orig).setEntity(null);

        // 2) Remove collected gems and extra lives
        ...
        gameBoard.getEntityCell(dest).setEntity(gameBoard.getPlayer());
    ...
    public void undoMove(final MoveResult prevMove) {
        Objects.requireNonNull(prevMove, "prevMove cannot be null");

        if (!(prevMove instanceof MoveResult.Valid.Alive alive)) {
            // Only valid & alive moves actually mutated the board
            return;
        }
        ...
        // 1) Remove player from dest
        gameBoard.getEntityCell(dest).setEntity(null);

        // 2) Restore collected gems & lives
        for (Position gemPos : alive.collectedGems) {
