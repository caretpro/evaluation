...
public class ConsolePlayer extends Player {
	...
	public Move nextMove(Game game, Move[] availableMoves) {
		...
		while (true) {
			...
			if (mv == null) {
				System.out.println("Invalid format. Correct example: a1->b3");
				continue;
			}
			Move chosen = null;
			for (Move cand : availableMoves) {
				if (cand.equals(mv)) {
					chosen = cand;
					break;
				}
			...
			if (chosen == null) {
				System.out.println("That move is not allowed. Pick one from:");
				for (Move cand : availableMoves) {
					System.out.println("  " + cand.getSource() + "->" + cand.getDestination());
				}
				continue;
			}
			return chosen;
