...
public class GameState {
    ...
    /**
     * Proper constructor (was mistakenly declared as void).
     *
     * @param map the original GameMap to base this state on.
     */
    public GameState(GameMap map) {
        this.entities = new HashMap<>();
        for (Map.Entry<Position, Entity> e : map.map.entrySet()) {
    ...
    public void checkpoint() {
        // Capture a deep‑copy of current transition history and undoQuota
    ...
    public Position getPlayerPositionById(int id) {
        for (var entry : entities.entrySet()) {
            ...
            if (e instanceof Player p && p.getId() == id) {
                return entry.getKey();
            }
        }
        throw new IllegalArgumentException("No player with id " + id + " found in game state.");
    }
    ...
    public Set<Position> getAllPlayerPositions() {
        return entities.entrySet().stream().filter(e -> e.getValue() instanceof Player).map(Map.Entry::getKey)
    ...
    /**
     * Get the undo quota currently left, i.e., the maximum number of undo actions that can be performed from now on. If undo is unlimited,
     *
     * @return the undo quota left (using {@link Optional#of(Object)}) if the game has an undo limit; {@link Optional#empty()} if the game has unlimited undo.
     */
    ...
    /**
     * Move the entity from one position to another. This method assumes the validity of this move is ensured. <b>The validity of the move of the entity in one position to another need not be checked.</b>
     *
    ...
    public void undo() {
        if (history.isEmpty()) {
            applyTransition(currentTransition.reverse());
        } else {
            Transition ckpt = history.pop();
            applyTransition(ckpt.reverse());
        }
        currentTransition = history.isEmpty() ? new Transition() : history.peek();
        if (undoQuota >= 0) {
            undoQuota--;
        }
    }
