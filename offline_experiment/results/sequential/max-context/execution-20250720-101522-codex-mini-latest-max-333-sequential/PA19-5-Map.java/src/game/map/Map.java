
package game.map.cells;

import game.MapElement;
import game.pipes.Pipe;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;
import util.Coordinate;

import java.util.Optional;

/**
 * Represents a {@link Cell} which can contain a pipe.
 */
public class FillableCell extends Cell implements MapElement {

    @Nullable
    private       Pipe pipe;  // make mutable

    /**
     * Constructs a {@link FillableCell} with no pipe.
     *
     * @param coord Coordinate where the cell resides in the map.
     */
    public FillableCell(@NotNull Coordinate coord) {
        super(coord);
        this.pipe = null;
    }

    /**
     * Constructs a {@link FillableCell} with an initial pipe.
     *
     * @param coord Coordinate where the cell resides in the map.
     * @param pipe  The pipe inside this cell, or null.
     */
    public FillableCell(@NotNull Coordinate coord, @Nullable Pipe pipe) {
        super(coord);
        this.pipe = pipe;
    }

    /**
     * @return whether a pipe is present in this cell.
     */
    public boolean hasPipe() {
        return pipe != null;
    }

    /**
     * Place the given pipe into this cell.
     *
     * @param pipe pipe to set; must be non-null
     */
    public void setPipe(@NotNull Pipe pipe) {
        this.pipe = pipe;
    }

    /**
     * Clears any pipe from this cell.
     */
    public void clearPipe() {
        this.pipe = null;
    }

    /**
     * @return an Optional representing the pipe in this tile.
     */
    public @NotNull Optional<Pipe> getPipe() {
        return Optional.ofNullable(pipe);
    }

    /**
     * @return The character representation of the pipe, or {@code '.'} if the cell is empty.
     */
    @Override
    public char toSingleChar() {
        return pipe != null ? pipe.toSingleChar() : '.';
    }
}