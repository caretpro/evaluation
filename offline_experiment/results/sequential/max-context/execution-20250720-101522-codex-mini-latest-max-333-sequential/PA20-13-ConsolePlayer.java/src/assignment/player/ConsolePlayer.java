...
public class ConsolePlayer extends Player {
	...
	public Move nextMove(Game game, Move[] availableMoves) {
		...
		while (true) {
			game.refreshOutput();
			System.out.print("Your move (format a1->b3): ");
			String line = scanner.nextLine().trim();
			Move candidate = parseMove(line);
			if (candidate == null) {
				System.out.println("  >> Invalid format. Please use a1->b3 style.");
				continue;
			}
			boolean found = false;
			...
			if (!found) {
				System.out.println("  >> That move is not in the list of legal moves.");
				continue;
			}
